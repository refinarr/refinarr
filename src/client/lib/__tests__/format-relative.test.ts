...
describe("formatRelative", () => {
  ...
  test("seconds for differences under 60s", () => {
    ...
    expect(formatRelative(NOW - 59_000)).toBe("59s ago");
  });

  test("minutes between 60s and 60m", () => {
    expect(formatRelative(NOW - 60_000)).toBe("1m ago");
    expect(formatRelative(NOW - 30 * 60_000)).toBe("30m ago");
  });
  ...
  test("clamps negative offsets to 0s", () => {
    expect(formatRelative(NOW + 5_000)).toBe("0s ago");
  });

  test("accepts Date instance", () => {
    expect(formatRelative(new Date(NOW - 10_000))).toBe("10s ago");
  });
...
describe("formatEta", () => {
  test("returns 'now' for zero or negative", () => {
    ...
    expect(formatEta(-5_000)).toBe("now");
  });
  ...
  test("hours and minutes when not whole", () => {
    expect(formatEta(1 * 60 * 60_000 + 30 * 60_000)).toBe("1h 30m");
  });
});
