...
export function radarrHandlers(
  ...
  if (opts.movies !== undefined) {
    handlers.push(
      http.get(`${base}/movie`, () => HttpResponse.json(opts.movies)),
    );
  }
  if (opts.movieFiles !== undefined) {
    handlers.push(
      http.get(`${base}/moviefile`, ({ request }) => {
        const url = new URL(request.url);
        const ids = url.searchParams.getAll("movieFileIds").map(Number);
        const idSet = new Set(ids);
        const filtered = (opts.movieFiles as Array<{ id: number }>).filter(
          (f) => idSet.has(f.id),
        );
  ...
  if (opts.qualityProfiles !== undefined) {
    handlers.push(
      http.get(`${base}/qualityprofile`, () =>
        HttpResponse.json(opts.qualityProfiles),
      ),
    );
  }
  if (opts.customFormats !== undefined) {
    handlers.push(
      http.get(`${base}/customformat`, () =>
        HttpResponse.json(opts.customFormats),
      ),
    );
  }
  handlers.push(
    http.post(`${base}/command`, async () => {
      opts.onCommand?.();
...
export function sonarrHandlers(
  instance: ArrInstance,
  opts: {
  ...
  if (opts.series !== undefined) {
    handlers.push(
      http.get(`${base}/series`, () => HttpResponse.json(opts.series)),
    );
  }
  if (opts.episodeFilesByseriesId !== undefined) {
    handlers.push(
      http.get(`${base}/episodefile`, ({ request }) => {
        const url = new URL(request.url);
  ...
  if (opts.episodes !== undefined) {
    handlers.push(
      http.get(`${base}/episode`, () => HttpResponse.json(opts.episodes)),
    );
  }
  handlers.push(
    http.post(`${base}/command`, async () => {
