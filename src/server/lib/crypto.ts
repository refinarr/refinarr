...
function loadOrGenerateKey(): Buffer {
  ...
  if (envKey) {
    ...
    if (buf.length !== KEY_LEN) {
      throw new Error(`ENCRYPTION_KEY must decode to ${KEY_LEN} bytes (got ${buf.length}). Use a 32-byte base64 value.`);
    }
    return buf;
  }
  ...
  if (existsSync(path)) {
    ...
    if (buf.length !== KEY_LEN) {
      throw new Error(`Encryption key at ${path} is corrupt (length ${buf.length}, expected ${KEY_LEN}).`);
    }
    return buf;
  }
...
export function encryptSecret(plain: string): string {
  if (typeof plain !== "string") throw new Error("encryptSecret requires a string");
