      ...
      <SheetContent className="w-full sm:max-w-xl flex flex-col gap-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 text-sm">
          ...
          <div className="border-t pt-3">
            <h3 className="text-xs uppercase tracking-wide text-muted-foreground mb-2">{tDrawer("customFormats")}</h3>
            {(() => {
              if (!movie.hasFile) return <p className="text-muted-foreground text-sm">{tDrawer("noFileDownloaded")}</p>;
              if (hasCfs) return <CfScoreList formats={movie.customFormats} missingFormats={movie.missingFormats} />;
              return <p className="text-muted-foreground text-sm">{tDrawer("noCfData")}</p>;
            })()}
          </div>
        </div>
          ...
          {onDelete && movie.hasFile && movie.movieFileId > 0 && (
            <>
              <Button variant="outline" size="sm" onClick={() => onDelete(movie, false)}>
                <Trash2 className="h-4 w-4 mr-1 text-destructive" /> {tCommon("delete")}
              </Button>
              <Button variant="outline" size="sm" onClick={() => onDelete(movie, true)}>
                <Trash2 className="h-4 w-4 mr-1 text-destructive" /> {tDrawer("deleteAndSearch")}
              </Button>
            </>
          )}
