...
describe("POST /api/auth/setup", () => {
  test("creates the admin user, sets a session cookie, hashes the password", async () => {
    const res = await POST(makeReq({ username: "admin1", password: "TestPassword123!" }));
    expect(res.status).toBe(200);

    const user = await prisma.user.findUnique({ where: { username: "admin1" } });
  ...
  test("second setup attempt returns 409", async () => {
    ...
    const res = await POST(makeReq({ username: "second", password: "TestPassword123!" }, "2.2.2.2"));
  ...
  test("invalid username characters returns 400", async () => {
    const res = await POST(makeReq({ username: "has spaces", password: "TestPassword123!" }));
    expect(res.status).toBe(400);
  });
  ...
  test("11th attempt from same IP returns 429", async () => {
    ...
    for (let i = 0; i < 10; i += 1) {
      ...
      await POST(makeReq({ username: `u${i}`, password: "TestPassword123!" }, ip));
    }
    const res = await POST(makeReq({ username: "blocked", password: "TestPassword123!" }, ip));
