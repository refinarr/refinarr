...
const valid = { type: "radarr" as const, name: "MSW Radarr", url: baseUrl, apiKey: "abcd1234abcd1234abcd1234abcd1234" };
...
describe("POST /api/instances/[id]/test", () => {
  test("ok: true when upstream returns 200", async () => {
    ...
    const res = await testConnection(new NextRequest(`http://localhost/api/instances/${id}/test`, { method: "POST" }), ctxFor(id));
  ...
  test("ok: false when instance does not exist", async () => {
    const res = await testConnection(new NextRequest("http://localhost/api/instances/99999/test", { method: "POST" }), ctxFor(99999));
    expect(await res.json()).toEqual({ ok: false });
  });
});
...
describe("POST /api/instances/[id]/refresh", () => {
  test("clears cache entries scoped to the instance", async () => {
    ...
    const res = await refresh(new NextRequest(`http://localhost/api/instances/${id}/refresh`, { method: "POST" }), ctxFor(id));
    expect(res.status).toBe(200);
