...
function getReq(qs: string) {
  return new NextRequest(`http://localhost/api/preferences?${qs}`, { method: "GET" });
}
...
describe("PUT /api/preferences", () => {
  test("inserts CF preferences for the instance", async () => {
    const res = await PUT(putReq({
      instanceId: 1,
      cfs: [{ cfId: 10, cfName: "HDR" }, { cfId: 11, cfName: "Atmos" }],
    }), ctxNone);
    expect(res.status).toBe(200);
    expect(await prisma.cfPreference.count({ where: { instanceId: 1 } })).toBe(2);
  });

  test("empty cfs[] clears existing preferences", async () => {
    await PUT(putReq({ instanceId: 1, cfs: [{ cfId: 10, cfName: "HDR" }] }), ctxNone);
    await PUT(putReq({ instanceId: 1, cfs: [] }), ctxNone);
    expect(await prisma.cfPreference.count({ where: { instanceId: 1 } })).toBe(0);
  });
...
describe("GET /api/preferences", () => {
  test("returns only the requested instance's preferences", async () => {
    await PUT(putReq({ instanceId: 1, cfs: [{ cfId: 10, cfName: "HDR" }] }), ctxNone);
    await PUT(putReq({ instanceId: 2, cfs: [{ cfId: 20, cfName: "DV" }] }), ctxNone);
