...
function getReq(qs: string) {
  return new NextRequest(`http://localhost/api/ignore?${qs}`, { method: "GET" });
}

const validBody = { instanceId: 1, mediaId: 100, mediaType: "movie" as const, title: "Movie 100" };
...
describe("POST /api/ignore", () => {
  ...
  test("idempotent on (instanceId, mediaId, mediaType) — second POST returns same id", async () => {
    ...
    const b = await (await POST(postReq({ ...validBody, title: "Different" }), ctxNone)).json();
    expect(b.id).toBe(a.id);
  });

  test("schema-failing body returns 400", async () => {
    const res = await POST(postReq({ instanceId: 0, mediaId: 100, mediaType: "movie", title: "X" }), ctxNone);
...
describe("DELETE /api/ignore/[id]", () => {
  test("removes the entry", async () => {
    ...
    const res = await DELETE(new NextRequest(`http://localhost/api/ignore/${created.id}`, { method: "DELETE" }), ctxFor(created.id));
    expect(res.status).toBe(200);
    expect(await prisma.ignoreEntry.findUnique({ where: { id: created.id } })).toBeNull();
  });
});
