...
- **Search/delete dispatch is asynchronous.** `POST /api/radarr/movies/search` (and the Sonarr equivalents) returns `202 {"queued": true, ...}` meaning _accepted for processing_, not _completed_. A background worker drains the queue and talks to your *arr; the **terminal** outcome (success / failed) lands in **`GET /api/history`**. Check there for the real result rather than treating the `202` as success.
